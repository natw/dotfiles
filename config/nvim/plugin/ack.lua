...
local function selected_text()
  ...
  return table.concat(
    vim.api.nvim_buf_get_text(
      0,
      math.min(start_row, end_row) - 1,
      math.min(start_col, end_col) - 1,
      math.max(start_row, end_row) - 1,
...
vim.keymap.set("v", "<leader>ag", find_text(ag_cmd, selected_text), { noremap = true, silent = true })
vim.keymap.set("v", "<leader>ac", find_text(ack_cmd, selected_text), { noremap = true, silent = true })
vim.keymap.set("n", "<leader>ag", find_text(ag_cmd, cursor_text), { noremap = true, silent = true })
vim.keymap.set("n", "<leader>ac", find_text(ack_cmd, cursor_text), { noremap = true, silent = true })
